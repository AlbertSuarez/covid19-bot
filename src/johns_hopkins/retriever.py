...
from src.config import JOHNS_HOPKINS_USER_AGENT, JOHNS_HOPKINS_DATA_ATTEMPTS, JOHNS_HOPKINS_DATA_TIMEOUT, \
    JOHNS_HOPKINS_DATA_RTD
    ...
    for attempt in range(JOHNS_HOPKINS_DATA_ATTEMPTS):
        try:
            response = requests.get(
                data_url, headers={'User-Agent': JOHNS_HOPKINS_USER_AGENT}, timeout=JOHNS_HOPKINS_DATA_TIMEOUT
            )
